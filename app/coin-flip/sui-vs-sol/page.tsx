        ...
        <RecentCoinflipInteractsProvider coinFlipId={game.id.id}>
            <div className="w-full h-[calc(100vh-72px)] bg-[url(/coin-flip-background.png)]">
                <div className="flex flex-row p-20 h-full w-full">
                    ...
                    <div className="flex-1 flex justify-center items-center h-full w-full bg-muted rounded-l-md min-w-0">
                        <GameLauncher data={game} house_id={gameMetaData.house_id} stakes={gameMetaData.stakes} />
                    </div>
                    {/* Game Side Panel Container: Fixed width, no flex growth */}
                    <div className="relative flex-shrink h-full bg-background flex flex-col rounded-r-md border-l p-1 overflow-y-auto">
